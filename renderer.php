 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_sectionim_renderer extends plugin_renderer_base {
    ...
    public function display_section($course, $section, $sr, $level = 0, $pinned = false, $subsectionnumerator = null) {
        ...
        if ($level === 0) {
            ...
            // if (!$PAGE->user_is_editing()) {
                $topunit_btn = '<span class = "openall">'.get_string('openall', 'format_sectionim').'</span><spam class = "closeall">'.get_string('closeall', 'format_sectionim').'</span>';
                ...
                echo html_writer::tag('span', get_string('topunit', 'format_sectionim'), array('class' => 'topunit_name display__none'));
        ...
        if ($contentvisible) {
            ...
            if (!empty($children) || $movingsection) {
              ...
              // SG - hide temporary
                $sectionstyle = '';
                // if (course_get_format($course)->get_section($num)->collapsed == FORMAT_SECTIONIM_COLLAPSED && $level > 0 && !$PAGE->user_is_editing() ) {
                //   $sectionstyle = 'display:none';
                // }elseif (course_get_format($course)->get_section($num)->collapsed == FORMAT_SECTIONIM_COLLAPSED && $level > 0 && $PAGE->user_is_editing()) {
                //   $sectionstyle = 'display:none';
                // }

                if (course_get_format($course)->get_section($sectionnum)->collapsed == FORMAT_SECTIONIM_COLLAPSED && $level > 0)  $sectionstyle = 'display:none';
                ...
                foreach ($children as $num) {
                    if ($pinned) {
                        ...
                        if (course_get_format($course)->get_section($num)->pinned == FORMAT_SECTIONIM_PINNED) {
                            $this->display_insert_section_here($course, $section, $num, $sr);
                            $this->display_section($course, $num, $sr, $level+1);
                        }
                    } else {
                        if (course_get_format($course)->get_section($num)->pinned == FORMAT_SECTIONIM_UNPINNED) {
    ...
    /**
     * renders HTML for format_sectionim_edit_control
     *
     * @param format_sectionim_edit_control $control
     * @return string
     */
    protected function render_format_sectionim_edit_control(format_sectionim_edit_control $control) {
        if (!$control) {
            return '';
        }
        if ($control->class === 'movehere') {
            ...
            $icon = new pix_icon('mergeup', $control->text, 'format_sectionim', array('class' => 'iconsmall', 'title' => $control->text));
    ...
    public function confirm_delete_section($course, $sectionreturn, $deletesection) {
        ...
        echo $this->confirm(get_string('confirmdelete', 'format_sectionim'), $formcontinue, $formcancel);
        echo $this->box_end();
    }
}
