...
define('FORMAT_SECTIONIM_COLLAPSED', 0);
define('FORMAT_SECTIONIM_EXPANDED', 1);
define('FORMAT_SECTIONIM_PINNED', 1);
define('FORMAT_SECTIONIM_UNPINNED', 0);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_sectionim extends format_base {
    /**
    ...
    public function get_view_url($section, $options = array()) {
        ...
        if (array_key_exists('sr', $options)) {
            ...
            if ($section->collapsed == FORMAT_SECTIONIM_COLLAPSED) {
    ...
    protected function find_collapsed_parent($section, $returnid = false) {
        $section = $this->get_section($section);
        if (!$section->section || $section->collapsed == FORMAT_SECTIONIM_COLLAPSED) {
    ...
    public function course_format_options($foreditform = false) {
        ...
        $helprolessection['helpcontactroles_title'] = array(
            'label' => get_string('helpcontactroles_label', 'format_sectionim'),
            'element_type' => 'header',
        );
        foreach ($roles as $key => $value) { // Define roles list for help contact.
            if ($key != 16) { // Do not show Supporter role. It is used by default.
                $helprolessection['helpcontactroles_'.$key] = array(
                    ...
                    'help_component' => 'format_sectionim',
                );
            }
        }

        $config = get_config('format_sectionim');
        $courseformatoptions = array(
            'displayunits' => array(
                'label' => new lang_string('displayunits', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
            ...
            'displaymessages' => array(
                'label' => new lang_string('displaymessages', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        1 => new lang_string('yes'),
            ...
            'displaygrades' => array(
                'label' => new lang_string('displaygrades', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        1 => new lang_string('yes'),
            ...
            'showbagestag' => array(
                'label' => new lang_string('showbagestag', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        1 => new lang_string('yes'),
            ...
            'showcertificatestag' => array(
                'label' => new lang_string('showcertificatestag', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        1 => new lang_string('yes'),
        ...
        $maxcustomnumber = $DB->get_record('course_format_options', array('format' => 'sectionim', 'name' => 'customnumber', 'courseid' => $course->id), 'MAX(value) as maxnum'); // get DB query to find max custom section number
        ...
        $seclist += array(9999 => new lang_string('displayallsections', 'format_sectionim')); // add last item to select with value=9999 - conditionally unlimited sec number
        $courseformatoptions['displaysectionsnum'] = array(
            'label' => new lang_string('displaysectionsnum', 'format_sectionim'),
        ...
        $courseformatoptions['showcoursefullname'] = array(
            'label' => get_string('showcoursefullname', 'format_sectionim'),
            'element_type' => 'advcheckbox',
            'default' => 1,
            'element_attributes' => array(
                '',
    ...
    public function section_format_options($foreditform = false) {
        return array(
            ...
            'collapsed' => array(
                'type' => PARAM_INT,
                'label' => new lang_string('displaycontent', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_SECTIONIM_EXPANDED => new lang_string('showexpanded', 'format_sectionim'),
                        FORMAT_SECTIONIM_COLLAPSED => new lang_string('showcollapsed', 'format_sectionim'),
                    )
                ),
            ...
            'pinned' => array(
                'type' => PARAM_INT,
                'label' => new lang_string('displaypinned', 'format_sectionim'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_SECTIONIM_UNPINNED => new lang_string('showunpinned', 'format_sectionim'),
                        FORMAT_SECTIONIM_PINNED => new lang_string('showpinned', 'format_sectionim'),
                    )
                ),
                'default' => COURSE_DISPLAY_SINGLEPAGE,
            ),
     ...
     *
     * format_sectionim processes additional attributes in the view course URL
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if ($this->on_course_view_page()) {
            ...
            if ($switchcollapsed && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($section->collapsed == FORMAT_SECTIONIM_EXPANDED) {
                    $newvalue = FORMAT_SECTIONIM_COLLAPSED;
                } else {
                    $newvalue = FORMAT_SECTIONIM_EXPANDED;
                }
                $this->update_section_format_options(array('id' => $section->id, 'collapsed' => $newvalue));
                if ($newvalue == FORMAT_SECTIONIM_COLLAPSED) {
                    if (!isset($options['sr'])) {
            ...
            if ($switchpinned && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($section->pinned == FORMAT_SECTIONIM_UNPINNED) { // if section was not pinned before
                    if ($nowpinned < 4) {                           // if already pinned sections now < 4
                        $newvalue = FORMAT_SECTIONIM_PINNED;
                    } else if ($nowpinned >= 4) {
                        redirect(course_get_url($this->courseid, $switchcollapsed, $options), get_string('toomanypinned', 'format_sectionim'), null, \core\output\notification::NOTIFY_ERROR);
                    }
                } else {
                    $newvalue = FORMAT_SECTIONIM_UNPINNED;
                }
                $this->update_section_format_options(array('id' => $section->id, 'pinned' => $newvalue));
                if ($newvalue == FORMAT_SECTIONIM_PINNED) {
                    if (!isset($options['sr'])) {
                        $options['sr'] = $this->find_collapsed_parent($section->parent);
                    }
     ...
     *
     * @param int|section_info $section
     * @return array of format_sectionim_edit_control
     */
    public function get_section_edit_controls($section) {
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context) && $sectionnum != $sr) {
            ...
            if ($section->collapsed == FORMAT_SECTIONIM_EXPANDED) {
                $text = new lang_string('showcollapsed', 'format_sectionim');
                $class = 'expanded';
            } else {
                $text = new lang_string('showexpanded', 'format_sectionim');
                $class = 'collapsed';
            }
            $controls[] = new format_sectionim_edit_control($class, $switchcollapsedurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:setcurrentsection', $context)) {
            ...
            if ($course->marker == $section->section) {
                $marker = 0;
                $text = new lang_string('removemarker', 'format_sectionim');
                $class = 'marked';
            } else {
                $marker = $section->section;
                $text = new lang_string('setmarker', 'format_sectionim');
                $class = 'marker';
            }
            $setmarkerurl->params(array('marker' => $marker, 'sesskey' => sesskey()));
            $controls[] = new format_sectionim_edit_control($class, $setmarkerurl, $text);
        }

        // Set pin (mark section as persitent/pinned)
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            $switchpinnedurl = course_get_url($course, $sr);
            $switchpinnedurl->params(array('pinned' => $section->section, 'sesskey' => sesskey()));
            if ($section->pinned == FORMAT_SECTIONIM_UNPINNED) {
                $text = new lang_string('showpinned', 'format_sectionim');
                $class = 'unpinned';
            } else {
                $text = new lang_string('showunpinned', 'format_sectionim');
                $class = 'pinned';
            }
            $controls[] = new format_sectionim_edit_control($class, $switchpinnedurl, $text);
        }
        ...
        if (has_capability('moodle/course:update', $context)) {
            ...
            $controls[] = new format_sectionim_edit_control('settings', $editurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('mergeup', 'format_sectionim');
            $controls[] = new format_sectionim_edit_control('mergeup', $mergeupurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('deletesection', 'format_sectionim');
            $controls[] = new format_sectionim_edit_control('delete', $deleteurl, $text);
        }
    ...
    public function get_edit_control_movehere($parent, $before, $sr = null) {
        ...
        return new format_sectionim_edit_control('movehere', $movelink, $str);
    }

    /**
     * Returns a control to exit the section moving mode
     *
     * @return null|format_sectionim_edit_control
     */
    public function get_edit_controls_cancelmoving() {
        ...
        if ($movingsection) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_sectionim', $this->get_section_name($movingsection)));
            $controls[] = new format_sectionim_edit_control('cancelmovingsection', $cancelmovingurl, $str);
        }
        ...
        if (ismoving($this->courseid)) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_sectionim', $USER->activitycopyname));
            $controls[] = new format_sectionim_edit_control('cancelmovingactivity', $cancelmovingurl, $str);
        }
        return $controls;
    }
    ...
    public function get_add_section_control($parentsection) {
        ...
        if ($parentsection) {
            $text = new lang_string('addsubsection', 'format_sectionim');
        } else {
            $text = new lang_string('addsection', 'format_sectionim');
        }
        return new format_sectionim_edit_control('addsection', $url, $text);
    }
...
/*        if ($this->get_viewed_section()) {
            ...
            if ($section->parent) {
                ...
                $text = new lang_string('backtosection', 'format_sectionim', $this->get_section_name($section->parent));
                $url = $this->get_view_url($section->section, array('sr' => $sr,'parent'=>1));
            } else {
                $sr = 0;
                $text = new lang_string('backtocourse', 'format_sectionim', $this->get_course()->fullname);
                $url = $this->get_view_url($section->section, array('sr' => $sr));
            }

            return new format_sectionim_edit_control('backto', $url, strip_tags($text));
        }
        ...
        if ($PAGE->context && $PAGE->context->contextlevel == CONTEXT_MODULE && $PAGE->cm) {
            ...
            if ($sectionnum) {
                $text = new lang_string('backtosection', 'format_sectionim', $this->get_section_name($sectionnum));
            } else {
                $text = new lang_string('backtocourse', 'format_sectionim', $this->get_course()->fullname);
            }
            return new format_sectionim_edit_control('backto', $this->get_view_url($sectionnum), strip_tags($text));
        }
*/
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_sectionim');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_sectionim', $title);
        }
        if ($linkifneeded) {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_sectionim_edit_control implements renderable {
    public $url;
    public $text;
    public $class;
 ...
 * @return \core\output\inplace_editable
 */
function format_sectionim_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'sectionim'), MUST_EXIST);
 ...
 * Implements callback inplace_editable() allowing to edit values in-place
 *
 * @param array $stringurl
 * @return bool
 */
function format_sectionim_check_params() {

    //TODO REFACTOR RECURSION OF SUB SECTIONS
    if (optional_param('parent', 0, PARAM_INT)){
        return true;
    }
 ...
 */
function format_sectionim_get_fontawesome_icon_map() {
    return [
        'format_sectionim:mergeup' => 'fa-level-up',
    ];
}
